...
mod tests {
    ...
    #[test]
    fn canonical_arrays_retain_order_while_nested_objects_are_sorted() {
        let rendered =
            String::from_utf8(bytes(&json!([{"z": 1, "a": 2}, {"b": 3, "a": 4}])).unwrap())
                .unwrap();
        assert_eq!(rendered, r#"[{"a":2,"z":1},{"a":4,"b":3}]"#);
    }
