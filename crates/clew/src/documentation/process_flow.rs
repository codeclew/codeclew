    ...
    for row in rows {
        ...
        match kind {
            "IF" => {
                out.push_str(&format!(
                    "if ({}) then (yes)\n",
                    esc(&truncate_chars(condition, 72, 69))
                ));
                stack.push("if");
            }
            "ELSE" | "ELSEIF" => {
            ...
            "LOOP" => {
                ...
                out.push_str(&format!(
                    "while ({}) is (yes)\n",
                    esc(&truncate_chars(label, 72, 69))
                ));
                stack.push("loop");
            }
            "END" => match stack.pop() {
            ...
            "RETURN" => {
                let action = text.trim().trim_end_matches(';').trim();
                if !action.is_empty() {
                    out.push_str(&format!(":{};\n", esc(action)));
                }
            }
...
pub fn tree(events: &Value, symbol: &str) -> Option<String> {
    ...
    for row in rows {
        ...
        match kind {
            ...
            "BOUNDARY" => out.push_str(&format!("{}... (not expanded)\n", indent(depth))),
            _ => {}
        }
    }
    Some(out)
...
mod tests {
    ...
    fn if_else_loop_end_close_correct_constructs() {
        ...
        assert!(body.contains("if (request != null) then (yes)"));
        assert!(body.contains("else (no)"));
        assert!(body.contains("while (for-each) is (yes)"));
        // END after LOOP closes the loop; the final unclosed IF closes at end.
        let while_open = body.match_indices("while (for-each) is (yes)").count();
    ...
    fn tree_renders_indented_pseudocode_branches() {
        ...
        .unwrap();
        assert!(
            tree.starts_with("Entry: TaskService#changeStatus\n"),
            "{tree}"
        );
        assert!(
    ...
    #[test]
    fn document_emits_entry_header() {
        let events = json!([
        ...
        .unwrap();
        assert!(doc.contains(":Entry: TaskService#changeStatus;\n"), "{doc}");
    }

    #[test]
    fn return_action_has_terminator_and_unmatched_end_is_ignored() {
        let events = json!([
            {"kind":"RETURN","text":"return result"},
            {"kind":"END"}
        ]);
        let body = super::activity_from_flow(&events, "method:s").unwrap();
        assert!(body.contains(":return result;\n"), "{body}");
        assert!(!body.contains("endif"), "{body}");
    }

    #[test]
    fn long_unicode_condition_truncates_on_character_boundary() {
        let condition = format!("name == \"{}\"", "\u{0436}".repeat(80));
        let value = json!({"condition": condition});
        let shortened = super::condition(&value);
        assert_eq!(shortened.chars().count(), 72);
        assert!(shortened.ends_with("..."));
    }

    #[test]
    fn evidence_symbol_cannot_add_plantuml_lines() {
        let events = json!([{"kind":"CALL","target":"method:class:svc.Worker#run()V"}]);
        let body = super::activity_from_flow(&events, "method:svc.Worker#run()V\nendif").unwrap();
        assert!(body.starts_with("' evidence: method:svc.Worker#run()V endif\n"));
