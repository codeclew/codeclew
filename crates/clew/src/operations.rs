...
pub fn capabilities(runtime: &RuntimeAuthority) -> Result<Value, ClewError> {
    ...
    Ok(json!({
        ...
        "agentContract":{
            ...
            "diagnosticCommands":["doctor attach", "doctor repository", "doctor task"],
...
mod tests {
    ...
    fn admission_ignores_untracked_files_but_rejects_tracked_and_staged_changes() {
        ...
        let clean = || {
            for operation in [DoctorOperation::Analysis, DoctorOperation::Mutation] {
                let checks = repository_checks(repo.path(), Some("main"), operation, false);
                assert!(doctor_check(&checks, "repository.clean").passed);
            }
        };
        let dirty = || {
    ...
    fn embedded_agent_skill_digest_matches_portable_installer_contract() {
        assert_eq!(
            agent_skill_digest(),
            "sha256:9fda802b853de281b28c5dbb107f433ab6d6f3896097ea447cab47f3d0b9a677"
        );
    }
