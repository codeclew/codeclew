...
const RU_MESSAGES={
 ...
 "Detailed process steps": "Подробные шаги процесса",
 "State diagram": "Диаграмма состояний",
 "Activity on transitions": "Действия на переходах",
 "Source syntax summary": "Краткое описание синтаксиса исходного кода",
 "Retained structured FLOW": "Сохранённый структурированный FLOW",
 "PlantUML SVG preview is unavailable; the .puml file and text steps remain available.": "Предварительный просмотр PlantUML SVG недоступен; файл .puml и текст шагов сохранены.",
 "Declared process-state schema": "Объявленная схема состояний процесса",
 "Static source bindings do not show runtime execution.": "Статические привязки к коду не показывают выполнение программы.",
 "States and transitions come from the captured process-state schema.": "Состояния и переходы взяты из сохранённой схемы состояний процесса.",
 "One branch per declared lifecycle operation; this is not an observed runtime trace.": "Отдельная ветвь для каждой объявленной операции; фактическое выполнение не наблюдалось.",
 "Static source interpretation; runtime execution was not observed.": "Статическое объяснение исходного кода; выполнение программы не наблюдалось.",
...
const entries=D.catalogue.length?[...D.catalogue]:publication.operations.filter(o=>!o.id.startsWith('section-')&&!o.id.startsWith('assessment-')&&o.id!=='process-overview'&&!o.dataflow).map(o=>({id:o.id,symbol:translationGaps[o.id]?o.id:o.title,kind:'SCENARIO',trigger:{methods:['FLOW'],paths:[]},sourceIds:[]}));
if(D.process){for(const operation of D.operations.filter(o=>!o.id.startsWith('section-')&&!o.id.startsWith('assessment-')&&o.id!=='process-overview'&&!o.dataflow)){if(!entries.some(entry=>entry.id===operation.id))entries.push({id:operation.id,symbol:translationGaps[operation.id]?operation.id:operation.title,kind:'SCENARIO',trigger:{methods:['FLOW'],paths:[]},sourceIds:[]});}}
...
function plantumlPreview(stem,available,alt){return available?chromeHtml`<div class="diagram-img-wrap"><img class="diagram-img" src="../diagrams/${esc(stem)}.svg" alt="${esc(alt)}"></div>`:chromeHtml`<p class="renderer-unavailable">${esc(t('PlantUML SVG preview is unavailable; the .puml file and text steps remain available.'))}</p>`;}
