...
impl RepositoryIndex {
    pub fn open(repo: &Path) -> Result<Self, SthreadError> {
        ...
        let connection = Connection::open(state.join("index.sqlite3")).map_err(db_error)?;
        connection
    ...
    pub fn update(&mut self, facts: &Value) -> Result<String, SthreadError> {
        ...
        for file in files {
            ...
            tx.execute("INSERT INTO files(path,content_hash,facts_json) VALUES(?1,?2,?3) ON CONFLICT(path) DO UPDATE SET content_hash=excluded.content_hash,facts_json=excluded.facts_json", params![path, hash, canonical::bytes(file).map_err(internal)?]).map_err(db_error)?;
