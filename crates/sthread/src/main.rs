...
fn expand_task_recipe(plan: &mut Value, context: &Value) -> Result<(), SthreadError> {
    ...
    plan["operations"] = json!([
        ...
        },
        {
            ...
            "preconditions":{"substitutions":[{"old":"val entity: ProductCanonicalDto? = null","new":"val entity: io.private-product.pim.productrepo.dto.ProductChangeEntity? = null"}]}
        },
        {
            "kind":"REWRITE_DECLARATION","target":{"targetId":producer},
            "preconditions":{"substitutions":[{"old":"product: ProductCanonicalDto? = null","new":"product: io.private-product.pim.productrepo.dto.ProductChangeEntity? = null"}]}
        },
        {
            "kind":"REWRITE_DECLARATION","target":{"targetId":repository},
            "preconditions":{"substitutions":[
                {"old":"SELECT n.id FROM Nomenclature n WHERE n.id IN :ids AND n.title NOT LIKE 'ЪЫ%'","new":"SELECT new io.private-product.pim.productrepo.dto.ProductArchiveEventDto(n.id, n.code, n.title) FROM Nomenclature n WHERE n.id IN :ids AND n.title NOT LIKE 'ЪЫ%'"},
                {"old":"fun findNonTestIdsByIds(@Param(\"ids\") ids: List<UUID>): List<UUID>","new":"fun findNonTestArchiveEventProductsByIds(@Param(\"ids\") ids: List<UUID>): List<io.private-product.pim.productrepo.dto.ProductArchiveEventDto>"}
            ]}
        },
        {
        ...
        },
        {
            ...
            "preconditions":{"substitutions":[{
                "old":"eq(product.id),\n                product = anyOrNull(),\n                changes",
                "new":"eq(product.id),\n                product = argThat<io.private-product.pim.productrepo.dto.ProductChangeEntity> {\n                    id == product.id && code == product.code && title == product.title\n                },\n                changes"
            }]}
        }
    ]);
    Ok(())
