...
> **Scope:** Hybrid generation of process documentation — auto-render "Подробный порядок действий" from evidence (default), declarative state diagram, PlantUML output, evidence binding.

## Goal

Make codeclew **auto-generate** process descriptions that look like the validated mockup
(`~/repo/kasko/arch-kasko/generated-task-lifecycle.html`):
...
- A **"Подробный порядок действий"** per operation, auto-built from **FLOW_STEP evidence +
...
1. **Generation stage:** Hybrid.
   - Render auto-shows the flow from evidence as "Подробный порядок действий".
...
### Component 1 — Flow renderer ("Подробный порядок действий")
...
```
[*] --> WAIT
WAIT --> PROCESSING : воркер взял в работу [takeTaskForProcessing]
PROCESSING --> WAIT_DECISION : status → WAIT_DECISION [changeTaskStatus]
ERROR --> WAIT : restart (closeErrorsAndSetWaitStatus) [restartTask]
```

"Подробный порядок действий" (activity, generated from FLOW_STEP + source):
```
start
:Вход: changeTaskStatus(taskId, ChangeTaskStatusRequest);
if (anyTask) then (да)
  :taskHandlingService.changeAnyTaskStatus(...);
else (нет)
  :taskHandlingService.changeTaskStatus(...);
endif
if (FINISHED == taskInstance.getTaskStatus()) then (да)
...
### Slice A — Flow renderer (auto "Подробный порядок действий")
