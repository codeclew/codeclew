...
и Clew skill. Навигация сначала использует Codeclew/structured facts;
...
| --- | --- | --- |
| `B03-Q1` | Resolve `RepositoryIndex::stage_update` in `crates/clew/src/index.rs` and its local dependency | Unique declaration; call to `update_from_root`; staged/private database precedes publication |
| `B03-Q2` | Resolve `graph::slice` in `crates/clew/src/graph.rs` | Unique declaration; bounded node/deadline policy; DEF_USE implies PHI_INPUT; unsupported/external boundary remains partial |
| `B03-Q3` | Resolve `transaction::commit` in `crates/clew/src/transaction.rs` | Unique declaration; Git target ref and repository-index snapshot are both checked; recovery/idempotency path is visible |
...
| --- | --- | --- | --- |
| `B03-Q1` | `crates/clew/src/index.rs::index::tests::staged_index_is_invisible_until_atomic_publish` | `cargo test -p clew --lib index::tests::staged_index_is_invisible_until_atomic_publish -- --exact` | exit `0`; named test `ok`; capability matrix resolves ровно один `stage_update`, его call to `update_from_root` и publish-after-stage path |
| `B03-Q2` | embedded graph fixtures in `crates/clew/src/graph.rs`; `fixtures/kotlin-basic` via `crates/clew/tests/golden_language.rs` | `cargo test -p clew --lib graph::tests::adds_phi_def_use_and_control_dependencies -- --exact`; `cargo test -p clew --lib graph::tests::budget_is_explicitly_partial -- --exact`; `cargo test -p clew --test golden_language k2_fir_golden_language_and_slice_matrix -- --exact` | all exit `0`; all three named tests `ok`; assertions retain PHI/DEF_USE/control deps, budget partiality and external-boundary partiality |
| `B03-Q3` | `fixtures/kotlin-basic` copied by `crates/clew/tests/concurrency_matrix.rs::{mandatory_concurrency_matrix,callee_formatting_replays_and_same_import_merges_idempotently}` | `cargo test -p clew --test concurrency_matrix mandatory_concurrency_matrix -- --exact`; `cargo test -p clew --test concurrency_matrix callee_formatting_replays_and_same_import_merges_idempotently -- --exact` | both exit `0`; both named tests `ok`; assertions cover target-ref movement, stale reslice, recovered pre-CAS candidate and idempotent replay |
| `B03-U1` | embedded `A.kt` fixtures in three `crates/clew/src/index.rs` tests | `cargo test -p clew --lib index::tests::unchanged_files_are_not_rewritten -- --exact`; `cargo test -p clew --lib index::tests::classifies_body_summary_signature_and_abi_invalidation -- --exact`; `cargo test -p clew --lib index::tests::failed_stage_preserves_published_snapshot -- --exact` | all exit `0`; named tests `ok`; unchanged rewrite count is `0`, changed facts/invalidation set is exact, failed stage preserves published hash |
| `B03-T1` | `fixtures/kotlin-basic` copied by `crates/clew/tests/concurrency_matrix.rs::{mandatory_concurrency_matrix,callee_formatting_replays_and_same_import_merges_idempotently}` | `cargo test -p clew --test concurrency_matrix mandatory_concurrency_matrix -- --exact`; `cargo test -p clew --test concurrency_matrix callee_formatting_replays_and_same_import_merges_idempotently -- --exact` | both exit `0`; both named tests `ok`; `StaleRequiresReslice`, target-ref CAS recovery actions and idempotency assertions pass |
| `B03-W1` | `fixtures/kotlin-2-1`; `crates/clew/tests/kotlin21.rs::selects_matching_kotlin_21_worker_and_resolves_extension_names` | `cargo test -p clew --test kotlin21 selects_matching_kotlin_21_worker_and_resolves_extension_names -- --exact` | exit `0`; named test `ok`; exact `2.1.21` and `2.4.10` worker/compiler assertions pass |
| `B03-W2` | `fixtures/kotlin-maven`; two named tests in `crates/clew/tests/maven.rs` | `cargo test -p clew --test maven opens_maven_kotlin_23_project_with_exact_worker_and_build_plan -- --exact`; `cargo test -p clew --test maven indexes_and_resolves_maven_sources_with_k2 -- --exact` | both exit `0`; both named tests `ok`; Maven model and K2 resolution use exact `2.3.0` worker |
