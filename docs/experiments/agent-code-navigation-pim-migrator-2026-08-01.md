...
Terra-агент повторил режим `sthread` на том же baseline как `sthread v2`.

## Результаты

| Метрика | Default | ast-index | sthread v1 | sthread v2 |
|---|---:|---:|---:|---:|
...
- `sthread`: +24,2% времени, +79,1% raw total tokens и +28,5%
  некэшированных tokens.
- `sthread v2`: +24,2% времени, +195,6% raw total tokens и +52,5%
  некэшированных tokens.
...
### sthread v2 после Kotlin 2.1 worker и `context`
...
tokens на 18,7%, output tokens на 17,0% относительно sthread v1.

Трасса объясняет расхождение: v2 сделал 29 отдельных tool roundtrips против 19
у v1. Из 11 содержательных SThread-запросов шесть были неуспешными попытками
...
репозитории этим одиночным прогоном не доказано. `sthread v2` устранил блокеры
...
не менее чем пятью прогонами на режим. Для следующего SThread worker сначала
