...
> Historical proposal preserved on 2026-09-13. Shared JVM facts and Spring
> derivation subsequently shipped in 0.7.0 and 0.8.0. The observations below
> describe the archived working copy, not the current architecture. See the
> [consolidation record](../product/validation/repository-consolidation-20260913.md).
...
|---|---|---|
| E1 | All three Kotlin Spring readers mix compiler access with framework selection. The returned windows repeat nearest-family inheritance selection, controller detection and Feign exclusion. The 24 reader additionally exposes the full alias/value/hierarchy machinery. This is source-level duplication, not a measured equivalence claim. | `03-kotlin-open.json`, `05-kotlin-reader.json`; [24 reader](https://github.com/codeclew/codeclew/blob/17ed857/workers/kotlin/src/main/kotlin/dev/semanticthread/worker/SpringAnnotationFacts24.kt), lines 51–349; [21 reader](https://github.com/codeclew/codeclew/blob/17ed857/workers/kotlin21/src/main/kotlin/dev/semanticthread/worker/SpringAnnotationFacts21.kt), lines 50–74 and 191–224; [23 reader](https://github.com/codeclew/codeclew/blob/17ed857/workers/kotlin23/src/main/kotlin/dev/semanticthread/worker/SpringAnnotationFacts23.kt), lines 51–73. |
| E2 | Rust already owns a Spring metadata type, validation and trigger projection. Metadata has annotation chains, raw attributes, runtime-conditional registration, optional target symbol and bean class. | `02-rust-open.json`; [spring_entrypoints.rs](https://github.com/codeclew/codeclew/blob/17ed857/crates/clew/src/spring_entrypoints.rs), lines 27–90 and 160–238. |
| E3 | The Rust Java declaration contract has `annotations: Vec<String>` plus separate optional `spring` and `documentation` payloads. A plain annotation-name array cannot carry the input needed to replace the existing Spring payload. This is Rust syntax evidence, not inspection of javac execution. | `06-java-contract.json`; [java_adapter_v2.rs](https://github.com/codeclew/codeclew/blob/17ed857/crates/clew/src/java_adapter_v2.rs), lines 34–128. |
| E4 | `LanguageAdapter` already exposes handshake, generation, cancellation and shutdown. The handshake includes adapter identity/digest, languages, capabilities and toolchains. The registry distinguishes build-model providers and language adapters. | `08-adapter-contract.json`; [adapter_v2.rs](https://github.com/codeclew/codeclew/blob/17ed857/crates/clew/src/adapter_v2.rs), lines 162–203 and 285–327. |
| E5 | Kotlin already uses one Rust adapter type parameterized by a semantic engine and generation driver. An existing fake-driver test describes a common streaming contract for 21/23/24; the test was read, not executed here. | `12-kotlin-adapter.json`; [kotlin_adapter_v2.rs](https://github.com/codeclew/codeclew/blob/17ed857/crates/clew/src/kotlin_adapter_v2.rs), lines 42–82 and 1408–1454. |
| E6 | Documentation `project` consumes language facts, preserves flow events, reads `fact.spring`, selects expected authority by service language, and builds entrypoint observations. IDs include target and ordinal; missing implementation bodies produce a boundary. | `11-docs-project.json`; [documentation/analysis.rs](https://github.com/codeclew/codeclew/blob/17ed857/crates/clew/src/documentation/analysis.rs), lines 447–641. |
| E7 | An existing derived **input** manifest binds repository snapshot, provider models and sealed inputs. It is a reusable storage pattern, not evidence that a framework-result cache already exists. | `10-derived.json`; [derived_manifest.rs](https://github.com/codeclew/codeclew/blob/17ed857/crates/clew/src/derived_manifest.rs), lines 9–33. |
| E8 | Build configuration shares worker sources and excludes version-specific FIR/Spring files; worker manifests already bind distributions by file digests and tree hashes. The workspace currently declares one Rust crate. The protobuf envelope carries versioned messages with canonical JSON response payloads and blob references. | Native configuration observations in `15-config-observations.json`; [Kotlin build](https://github.com/codeclew/codeclew/blob/17ed857/workers/kotlin/build.gradle.kts), [21 build](https://github.com/codeclew/codeclew/blob/17ed857/workers/kotlin21/build.gradle.kts), [23 build](https://github.com/codeclew/codeclew/blob/17ed857/workers/kotlin23/build.gradle.kts), [worker manifests](https://github.com/codeclew/codeclew/blob/17ed857/workers/manifests), [Cargo workspace](https://github.com/codeclew/codeclew/blob/17ed857/Cargo.toml), [worker protocol](https://github.com/codeclew/codeclew/blob/17ed857/schemas/worker.proto). |
