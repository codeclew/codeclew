...
internal fun springAnnotationFacts23(function: FirSimpleFunction, context: CheckerContext): JsonObject =
    SpringAnnotationReader23(context).read(function)

internal fun springClassAnnotationFacts23(owner: FirRegularClass, context: CheckerContext): JsonObject =
    SpringAnnotationReader23(context).readInherited(owner)
...
private data class SpringBinding23(
    val annotation: String,
    val chain: List<String>,
    val attributes: Map<String, JsonElement>,
)

private class SpringAnnotationReader23(private val context: CheckerContext) {
    ...
    fun read(function: FirSimpleFunction, beanOwner: FirRegularClass? = null): JsonObject {
        ...
        fun entry(binding: SpringBinding23, kind: String, extra: Map<String, JsonElement> = emptyMap()) {
        ...
        if (owner.status.modality != org.jetbrains.kotlin.descriptors.Modality.ABSTRACT &&
            ...
            scope.getCallableNames().sortedBy { it.asString() }.forEach { name ->
                scope.processFunctionsByName(name) { symbol ->
                    ...
                    val reader = SpringAnnotationReader23(context)
                    ...
                    val descriptor = compilerJvmMethodDescriptor23(method)
    ...
    private fun expandAll(annotations: List<FirAnnotation>): List<SpringBinding23> =
        annotations.flatMap { expand(it, emptyList()) }

    private fun expand(annotation: FirAnnotation, path: List<String>): List<SpringBinding23> {
        ...
        HTTP_METHODS[id.removePrefix(WEB)]?.takeIf { id.startsWith(WEB) }?.let { method ->
            return listOf(SpringBinding23(WEB + "RequestMapping", chain, aliases(attributes) + ("method" to JsonArray(listOf(JsonPrimitive(method))))))
        }
        if (id in ROOT_ANNOTATIONS) return listOf(SpringBinding23(id, chain, if (id == WEB + "RequestMapping") aliases(attributes) else attributes))
