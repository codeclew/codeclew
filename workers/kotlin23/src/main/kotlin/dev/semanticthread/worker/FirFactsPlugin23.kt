...
private class FirFactsConstructorDescriptorChecker(
    ...
    override fun check(declaration: FirConstructor) {
        ...
        val jvmDescriptor = compilerJvmMethodDescriptor23(declaration)
...
internal fun compilerJvmMethodDescriptor23(declaration: FirFunction): String? = runCatching {
...
private data class ResolvedCallableTarget23(
    val compilerCallableId: String,
    val symbolIdentity: String,
    val jvmDescriptor: String,
)

private data class ResolvedCallableTargetResult23(
    val target: ResolvedCallableTarget23?,
    val unknownCode: String?,
)

/** Exact FIR-selected target identity; never reconstructed from a name or arity. */
private fun resolvedCallableTarget23(
    callable: FirCallableSymbol<*>,
    session: FirSession,
): ResolvedCallableTargetResult23 {
    val callableId = callable.callableId
        ?: return ResolvedCallableTargetResult23(null, "NO_COMPILER_CALLABLE_ID")
    if (callableId.isLocal) {
        return ResolvedCallableTargetResult23(null, "LOCAL_CALL_TARGET_UNSUPPORTED")
    }
    val function = callable.fir as? FirFunction
        ?: return ResolvedCallableTargetResult23(null, "CALL_TARGET_NOT_FUNCTION")
    if (!function.origin.fromSource || function.origin.generated) {
        return ResolvedCallableTargetResult23(null, "EXTERNAL_OR_GENERATED_CALL_TARGET")
    }
    if (function.hasAnnotationSafe(JvmStandardClassIds.Annotations.JvmName, session)) {
        return ResolvedCallableTargetResult23(null, "JVM_NAME_OVERRIDE_UNSUPPORTED")
    }
    val prefix = when (callable) {
        is FirConstructorSymbol -> "constructor:"
        is FirNamedFunctionSymbol -> "callable:"
        else -> return ResolvedCallableTargetResult23(null, "UNSUPPORTED_CALLABLE_SYMBOL_KIND")
    }
    val descriptor = compilerJvmMethodDescriptor23(function)
        ?: return ResolvedCallableTargetResult23(null, "UNRESOLVED_TARGET_JVM_DESCRIPTOR")
    val compilerId = callableId.toString()
    return ResolvedCallableTargetResult23(
        ResolvedCallableTarget23(
...
private fun resolvedArgumentMapping23(
    ...
    if (function.status.isSuspend) {
        return ResolvedArgumentMapping23(null, "SUSPEND_ARGUMENT_MAPPING_UNSUPPORTED")
    }
    ...
    for ((argument, parameter) in resolved.mapping.entries) {
        ...
        val argumentSource = argument.source
            ?: return ResolvedArgumentMapping23(null, "MISSING_ARGUMENT_SOURCE")
    ...
    if (omitted.any { function.valueParameters[it].defaultValue == null }) {
        return ResolvedArgumentMapping23(null, "UNMAPPED_NON_DEFAULT_PARAMETER")
    }
    return ResolvedArgumentMapping23(
...
private class FirFactsCfgChecker(
    ...
    override fun check(declaration: FirFunction) {
        ...
        val record = buildJsonObject {
            ...
            compilerJvmMethodDescriptor23(declaration)?.let { put("jvmDescriptor", it) }
            put(
                "returnType",
        ...
        if (access != null) {
            if (owner == null || callable == null) {
                ...
                if (relationKind == "CALLS" || relationKind == "CONSTRUCTS") {
                    val function = callable.fir as? FirFunction
                    val descriptor = function?.let(::compilerJvmMethodDescriptor23)
                ...
                if (relationKind != null) {
                    ...
                    val exactTarget = if (
                        ...
                        resolvedCallableTarget23(callable, context.session)
                    } else {
                        ResolvedCallableTargetResult23(null, null)
                    }
                    ...
                    if (relationKind == "CALLS" || relationKind == "CONSTRUCTS") {
                        ...
                        relation["omittedDefaultParameterIndices"] = kotlinx.serialization.json.JsonArray(
                            argumentMapping.omittedDefaultParameterIndices.orEmpty().map(::JsonPrimitive),
                        )
                    }
