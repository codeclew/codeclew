...
internal fun springAnnotationFacts21(function: FirSimpleFunction, context: CheckerContext): JsonObject =
    SpringAnnotationReader21(context).read(function)

internal fun springClassAnnotationFacts21(owner: FirRegularClass, context: CheckerContext): JsonObject =
    SpringAnnotationReader21(context).readInherited(owner)
...
private data class SpringBinding21(
    val annotation: String,
    val chain: List<String>,
    val attributes: Map<String, JsonElement>,
)

private class SpringAnnotationReader21(private val context: CheckerContext) {
    private val session: FirSession = context.session
    private val boundaries = sortedSetOf<String>()
    private var visits = 0

    fun read(function: FirSimpleFunction, beanOwner: FirRegularClass? = null): JsonObject {
        ...
        fun entry(binding: SpringBinding21, kind: String, extra: Map<String, JsonElement> = emptyMap()) {
            ...
            scope.getCallableNames().sortedBy { it.asString() }.forEach { name ->
                scope.processFunctionsByName(name) { symbol ->
                    ...
                    while (!target.fir.origin.fromSource && target.callableId.classId == owner.symbol.classId && visited.add(target)) {
                        val bases = scope.getDirectOverriddenMembers(target)
                    ...
                    val reader = SpringAnnotationReader21(context)
                    ...
                    val descriptor = compilerJvmMethodDescriptor21(method)
    ...
    private fun inheritedAnnotations(
        function: FirSimpleFunction, owner: FirRegularClass?, seen: MutableSet<String>,
    ): List<SpringBinding21> {
        ...
        val bases = scope.getDirectOverriddenMembers(function.symbol)
        return bases.flatMap { symbol ->
            val base = symbol.fir as? FirSimpleFunction ?: return@flatMap emptyList()
    ...
    private fun expandAll(annotations: List<FirAnnotation>): List<SpringBinding21> =
        annotations.flatMap { expand(it, emptyList()) }

    private fun expand(annotation: FirAnnotation, path: List<String>): List<SpringBinding21> {
        ...
        HTTP_METHODS[id.removePrefix(WEB)]?.takeIf { id.startsWith(WEB) }?.let { method ->
            return listOf(SpringBinding21(WEB + "RequestMapping", chain, aliases(attributes) + ("method" to JsonArray(listOf(JsonPrimitive(method))))))
        }
        if (id in ROOT_ANNOTATIONS) return listOf(SpringBinding21(id, chain, if (id == WEB + "RequestMapping") aliases(attributes) else attributes))
        ...
        return metas.map { meta ->
            ...
            members.forEach { (name, aliases) -> aliases.forEach { alias ->
                val target = alias.getKClassArgument(org.jetbrains.kotlin.name.Name.identifier("annotation"), session)
                    ?.classId?.asSingleFqName()?.asString()
                val targetName = alias.getStringArgument(org.jetbrains.kotlin.name.Name.identifier("attribute"), session)
                    ?.takeIf(String::isNotEmpty)
                    ?: alias.getStringArgument(org.jetbrains.kotlin.name.Name.identifier("value"), session)?.takeIf(String::isNotEmpty)
                    ?: name
    ...
    private fun aliasDestination(annotation: String, attribute: String, root: String, seen: MutableSet<String>): String? {
        ...
        val target = alias.getKClassArgument(org.jetbrains.kotlin.name.Name.identifier("annotation"), session)?.classId?.asSingleFqName()?.asString()
            ?.takeUnless { it == "kotlin.Annotation" || it == "java.lang.annotation.Annotation" } ?: annotation
        val name = alias.getStringArgument(org.jetbrains.kotlin.name.Name.identifier("attribute"), session)?.takeIf(String::isNotEmpty)
            ?: alias.getStringArgument(org.jetbrains.kotlin.name.Name.identifier("value"), session)?.takeIf(String::isNotEmpty) ?: attribute
    ...
    private fun elements(expression: FirExpression): List<FirExpression> = when (expression) {
        ...
        is FirArrayLiteral -> expression.argumentList.arguments.flatMap(::elements)
    ...
    private fun value(expression: FirExpression, depth: Int): JsonElement {
        ...
        when (expression) {
            ...
            is FirArrayLiteral -> return JsonArray(expression.argumentList.arguments.map { value(it, depth + 1) })
