...
private fun descriptorContainment(
    context: CheckerContext,
    declaration: Any,
): List<String> = context.containingDeclarations
...
private class FirFactsFunctionDescriptorChecker(
    ...
    override fun check(
        ...
        record["jvmAnnotations"] = jvmAnnotationFacts21(declaration, context)
...
private class FirFactsConstructorDescriptorChecker(
    ...
    override fun check(
        ...
        val ownerClass = context.containingDeclarations
...
private class FirFactsPropertyDescriptorChecker(
    ...
    override fun check(
        ...
        if (declaration.isLocal || callableId.isLocal) {
...
private class FirFactsClassDescriptorChecker(
    ...
    override fun check(
        ...
        record["jvmAnnotations"] = jvmClassAnnotationFacts21(declaration, context)
        record["typeParameters"] = kotlinx.serialization.json.JsonArray(typeParameters)
        appendFact(output, kotlinx.serialization.json.JsonObject(record))
    }
}
...
private fun resolvedArgumentMapping21(
...
): ResolvedArgumentMapping21 {
    val function = callable.fir as? FirFunction
        ?: return ResolvedArgumentMapping21(null, "ARGUMENT_OWNER_NOT_FUNCTION")
    if (!function.origin.fromSource || function.origin.generated || callable.callableId.isLocal) {
        return ResolvedArgumentMapping21(null, "EXTERNAL_OR_LOCAL_ARGUMENT_TARGET")
    }
    if (function.receiverParameter != null) {
        return ResolvedArgumentMapping21(null, "EXTENSION_ARGUMENT_MAPPING_UNSUPPORTED")
    }
    ...
    val resolved = arguments
        ?: return ResolvedArgumentMapping21(null, "MISSING_RESOLVED_ARGUMENT_MAPPING")
...
private class FirFactsOverrideChecker(
    ...
    override fun check(
        declaration: org.jetbrains.kotlin.fir.declarations.FirSimpleFunction,
        context: CheckerContext,
        reporter: DiagnosticReporter,
    ) {
        if (!declaration.status.isOverride) return
        val source = declaration.source ?: return
        val containingClass = context.containingDeclarations.filterIsInstance<FirRegularClass>().lastOrNull()
        val scope = containingClass?.symbol?.unsubstitutedScope(
            context.session,
        ...
        val bases = scope.getDirectOverriddenMembers(symbol)
        if (bases.isEmpty()) {
...
private data class ReturnBodyShape(
...
private fun returnBodyShape(function: FirFunction): ReturnBodyShape {
    val shape = ReturnBodyShape()
...
private fun resolvedReturnValueOccurrences(expression: FirExpression): List<FirCallableSymbol<*>> {
...
private fun returnDiagnosticHash(value: String): String = "sha256:" +
    MessageDigest.getInstance("SHA-256")
        .digest(value.toByteArray())
        .joinToString("") { byte -> "%02x".format(byte) }

private fun cfgReaches(
...
private fun emitDirectReturnValueFact(
    output: String,
    ...
    val diagnosticOccurrences = resolvedReturnValueOccurrences(result)
    fun refuse(code: String) {
        ...
        boundary["ownerIdentityHash"] = JsonPrimitive(
            returnDiagnosticHash(ownerId?.toString() ?: "<unknown>"),
        )
        boundary["rootFirKindHash"] = JsonPrimitive(
            returnDiagnosticHash(result::class.qualifiedName ?: "<unknown>"),
        )
        ...
        boundary["nestedResolvedOccurrenceKindHashes"] = kotlinx.serialization.json.JsonArray(
            diagnosticOccurrences
                .map { occurrence ->
                    returnDiagnosticHash(occurrence::class.qualifiedName ?: "<unknown>")
                }
                .sorted()
                .map(::JsonPrimitive),
        )
        appendFact(
            output,
            kotlinx.serialization.json.JsonObject(boundary),
        )
    }
    ...
    val bodyShape = returnBodyShape(owner)
    ...
    if (target.callableId.isLocal || !target.fir.origin.fromSource || target.fir.origin.generated) {
    ...
    val sourceReachesReturn = cfgReaches(sourceNode, returnNode)
    val sourceDominatesReturn = sourceReachesReturn
        && !cfgReaches(graph.enterNode, returnNode, sourceNode)
...
private fun emitNullCoalescingFact(
    ...
    if (unknown != null) {
        appendFact(
            output,
            unknownRelation(context, source, owner, "NULL_POLICY", unknown),
        )
        return
    }
...
private class FirFactsExpressionChecker(
    ...
    override fun check(
        ...
        if (expression is FirReturnExpression) {
            emitDirectReturnValueFact(output, expression, context)
        }
        val source = expression.source ?: return
        val owner = relationOwner(context)
            ...
            if (owner == null || callable == null) {
                ...
                if (relationKind != null) {
                    val argumentMapping = if (relationKind == "CALLS" || relationKind == "CONSTRUCTS") {
                        resolvedArgumentMapping21(callable, arguments)
                    } else {
                        ResolvedArgumentMapping21(emptyList(), null)
                    }
        ...
        val fact = buildJsonObject {
            ...
            putJsonArray("argumentToParameter") {
                if (value is FirFunctionCall && callable != null) {
                    resolvedArgumentMapping21(callable, arguments).rows.orEmpty().forEach(::add)
                }
            }
        }
