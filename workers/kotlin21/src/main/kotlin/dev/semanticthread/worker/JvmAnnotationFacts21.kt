...
internal fun jvmAnnotationFacts21(function: FirSimpleFunction, context: CheckerContext): JsonObject =
    JvmAnnotationReader21(context).read(function)

internal fun jvmClassAnnotationFacts21(owner: FirRegularClass, context: CheckerContext): JsonObject =
    JvmAnnotationReader21(context).readInherited(owner)

/** Compiler observations only; no framework names, aliases or registration rules. */
private class JvmAnnotationReader21(private val context: CheckerContext) {
    ...
    private fun identity(function: FirSimpleFunction): String? = compilerJvmMethodDescriptor21(function)
    ...
    private fun method(function: FirSimpleFunction, owner: FirRegularClass?, seen: MutableSet<String>): JsonObject {
        ...
        if (seen.size >= 128) boundaries += "METHOD_HIERARCHY_LIMIT"
            ...
            scope.getDirectOverriddenMembers(function.symbol).forEach { symbol ->
