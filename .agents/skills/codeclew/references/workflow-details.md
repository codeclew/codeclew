...
Read only the section needed for the current operation. The compact entrypoint
in `../SKILL.md` selects the task workflow.
...
[service documentation](service-documentation.md) and use its
...
When exact Rust source shows a `match` in a helper that controls an open
checklist item, select that helper once with exact `--term`, `--file`, and
`--source` instead of relying on incidental file visibility. If its retained
fact includes `matchArmCases`, add every returned `caseId` to the private
evidence checklist. Before the final answer, either draft and emit the exact
syntax-observable outcome for every case, or retain that case as an explicit
unproven obligation. `VISIBLE_PARSED_MATCH_ARMS_PARTIAL` never supports an
exhaustive claim. These facts do not prove runtime reachability or branch
semantics.
