...
class AgentSkillTest(unittest.TestCase):
    ...
    def test_agent_contract_is_installed_release_only(self) -> None:
        ...
        skill += (CANONICAL_SKILL / "references/workflow-details.md").read_text(encoding="utf-8")
        ...
        self.assertIn("private evidence checklist", skill)
        self.assertIn("At the retained-reference step", skill)
        self.assertIn("comparison predicate, typed outcome", skill)
        self.assertIn("Use this command-selection loop", skill)
        self.assertIn("one to three same-file declarations", skill)
        self.assertIn("Never request a facet to rediscover helpers", skill)
        self.assertIn("one evidence or claim entry per checklist item", skill)
        self.assertIn("codeclew-navigation-actions/1.0", skill)
        self.assertIn("sameFileRequired=true", skill)
        ...
        self.assertIn("fact includes `matchArmCases`", skill)
        self.assertIn("every returned `caseId`", skill)
        ...
        self.assertIn("one draft atom for every distinct visible", skill)
        self.assertIn("including cases whose value or order is preserved", skill)
        self.assertIn("Verify that modified", skill)
        self.assertIn("preserved cases are each `EMITTED`", skill)
        self.assertIn("generic recursive summary", skill)
        self.assertIn("--reference <terminal-or-full-path>", skill)
        self.assertIn("targetResolution=UNRESOLVED", skill)
