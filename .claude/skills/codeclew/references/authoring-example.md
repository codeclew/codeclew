...
summary describes the outcome in plain language; implementation details live in
the source inspector. A service operation ID is its exact catalogue entrypoint
ID; a scenario operation ID is its registered scenario ID.

```json
{
  "schema": "codeclew-documentation-narrative/1.0",
...
Allowed event kinds are `message`, `return`, `note`, `alt`, `else`, `loop`, `end`,
...
A corresponding reverse return also names that interaction. Nest receiver
